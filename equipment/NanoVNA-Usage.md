...
- Select the right `mode`/clock generator chip to reduce artifacts
  - Older versions of the NanoVNA use Si5351 chips, while newer ones use MS5351 chips. To identify which chip your model has, look for an `_MS` suffix on the [hardware version label](https://groups.io/g/nanovna-users/message/31631) on the back of the NanoVNA. For example, `HW version: 4.3_MS` indicates a MS5351 model.
