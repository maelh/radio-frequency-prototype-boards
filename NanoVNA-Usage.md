...
The power mode used during calibration is indicated by `Pa` which is short for `Power Auto`. Other possible values are `P2`, `P4`, `P6`, and `P8`. See menu entry `CALIBRATE>POWER AUTO` or `CALIBRATE>POWER <number>mA`. If the power mode is changed after calibration, the Pa/P<number> indicator is shown in red.
