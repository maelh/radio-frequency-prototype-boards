...
If one (or more) of the indicators are missing, calibration for that part is not present/was reset/not loaded. If one indicator is shown in lower case or another color, the currently loaded calibration is only of limited validity, since some parameter the calibration depends on was changed after calibration, such as the stimulus settings.
