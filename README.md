...
In the production files (if you chose to review them), diameters of PTHs will be changed, but that is to account for the narrowing of the holes when copper plating them and is normal. Otherwise, I saw other minor dimension changes of pads, but its mostly cosmetic: the outer ground pad of the U.FL connector was enlarged and moved a bit and is now protruding from the rectangular frame. I mostly would like to know why it happened, so I can account for it in other boards/situations where it might be relevant, but it was not worth the effort to keep delaying production.
